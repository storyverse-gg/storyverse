  ...
  return (
    <div>
      <div className="dropdown dropdown-end">
        <label tabIndex={0} className="btn btn-glass">
          {addr ?? "No Address"}
        </label>
        <ul tabIndex={0} className="mt-3 z-[1] p-2 shadow menu menu-sm dropdown-content bg-base-100 rounded-box w-52">
          ...
          <li><a>Settings</a></li>
          <li><a onClick={fcl.unauthenticate}>Logout</a></li>
        </ul>
      </div>
    </div>
  )
...
const UnauthenticatedState = () => {
  return (
    <div>
      ...
      <button className="btn ml-2" onClick={fcl.signUp}>Sign Up</button>
    </div>
  )
}
...
const Header = () => {
  ...
  return <div className="navbar backdrop-blur-md flex flex-1 justify-center p-4 fixed top-0 z-50">
    <div className="flex-1">
      <div className="w-96">
        <Link href={"/"}><Image alt="logo of storyverse" src={"/logo.png"} height={"80"} layout='responsive' width={"80"} /></Link>
      </div>
    </div>
    <div className="flex-none">
      {/* <label className="swap swap-rotate mr-2">
        <input onClick={(d) => toggleTheme()} value={theme == "cupcake" ? 1 : 0} type="checkbox" />
